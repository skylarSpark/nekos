...
_start:
_16_mode:
	mov	bp,0x20		; refering 16bit data segment in GDT
	mov	ds, bp
	mov	es, bp
	mov	ss, bp
	mov	ebp, cr0
	and	ebp, 0xfffffffe
	mov	cr0, ebp	; cr0.p = 0 turn off protected mode
	jmp	0:real_entry
real_entry:
	mov bp, cs
	mov ds, bp
	...
	mov bp,func_table
	add bp,ax
	call [bp]	; call C function from function table
	cli
	call disable_nmi
	mov	ebp, cr0
	or	ebp, 1
...
_getmmap:
	...
	mov edi,E80MAP_ADR
loop:
	mov eax,0e820h		; get e820map structure from 0e820h
	mov ecx,20			; e820map structure size
	mov edx,0534d4150h
	int 15h
	jc .1
